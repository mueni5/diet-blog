...
const PortalCalendar = ({ docAppointments, user, patientAppts, patients, patientNames }) => {
  ...
  const commitChanges = ({ added, changed, deleted }) => {
    if(changed){
      ...
      dispatch(updateBEAppointment(apptObj))
       //To update both front end and back end, had to create to seperate functions in the appointmentsSlice component,
       //one is a reducer in the createSlice() method the other is an extra reducer using a traditional fetch.
       //This was only way for the updates to both render in the front end and persist in the backend.
